...
Deno.test(
  'getUser error -> 401, jwt spogliato del Bearer, no delete',
  async () => {
    const { deps, getUser, deleteUser } = buildDeps({
...
Deno.test(
  'body vecchio con appleAuthCode -> ignorato, cancellazione riuscita',
  async () => {
    const { deps, deleteUser } = buildDeps({ user: emailUser });
    const res = await handler(
