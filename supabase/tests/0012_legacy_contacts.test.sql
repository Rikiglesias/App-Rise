...
-- venisse caricato non rivendica nulla se ha già il profilo completo — il trigger di
-- aggancio ripassa solo sul percorso di COMPLETAMENTO (`upsert`, T9 della suite 0014),
-- mentre la modifica dei dati usa `.update()` e non fa scattare nessun BEFORE INSERT
-- (T13 della 0014 lo prova). Qui l'utente non salva mai il profilo, quindi la riga
-- resta orfana. Se cancella l'account, la cascata su `claimed_by` non vede
