...
Dalla cartella `supabase/`, con Docker attivo. Sostituire `<SHIM>` con uno dei due file.
...
cat tests/<SHIM> \
    migrations/000*.sql \
    migrations/0008_partner_refs.sql \
    tests/0008_partner_refs.test.sql \
  | docker exec -i pgtest psql -U postgres -v ON_ERROR_STOP=1

docker rm -f pgtest
```

Nota: `0008` compare **due volte** nella pipe (una dentro `migrations/000*.sql`, una esplicita).
È voluto — è il test `T7`, che verifica la rieseguibilità della migration e che la seconda passata
non duplichi policy, trigger o indici. Chi applica la migration lo fa incollandola a mano nel SQL
...
anche se le righe precedenti erano verdi. Conteggio atteso: **14 righe con `PASS`** — 13 test
(`T1, T1b, T2, T3, T4, T5, T6a-T6f, T7`) più la riga di esito, che è sempre l'ultima.
