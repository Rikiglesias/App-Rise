...
| **D2-bis** | ⚠️ **Lo stesso buco esiste GIÀ OGGI, in app** — non è un problema solo del flusso web futuro | Chi entra con Apple o Google crea `auth.users` subito, ma profilo e `consent_events` nascono solo se completa «Completa profilo» (`useProfileForm.ts`). **Chi abbandona a metà resta senza `privacy_consent_at` e senza prova di consenso**, con un account già esistente. Combacia con l'anomalia già osservata sul database: 4 consensi marketing e **zero** `privacy_notice` | Che nessun account esista senza la prova del consenso che lo legittima | 🔧 raccogliere consenso e 18+ **prima** di creare l'utente, non dopo · oppure 🔧 purge programmato degli account che restano senza profilo oltre una soglia · 🔑 decidere quale delle due (la prima è più corretta, la seconda meno invasiva sul flusso esistente) | Query: `auth.users` senza riga in `profiles` = 0, oppure ogni residuo ha meno di N giorni |
