...
> nostro form, che raccoglie consenso e 18+ prima di creare l'utente — è un miglioramento, non una
...
| **B2-bis** | *(il nodo)* Peso dell'onboarding | **Due vincoli distinti, da non confondere.** Lato **form**: 11 voci obbligatorie (asse 11) [V, `validation.ts:65-85`]. Lato **database**: `profiles` ha NOT NULL su `first_name`, `last_name`, `phone`, `city`, `birth_date`, `privacy_consent_at` + constraint 18+ [V, `0001_profiles.sql:4-17`, `province` reso nullable da `0007`]. Il form di LD chiede una manciata di voci (email, password, paese, consenso) [V] | Che chi vuole comprare una gift card non incontri il modulo più lungo della sua giornata | 🔧 **migration 0010**: rendere nullable `phone` e `city` (`last_name` **resta obbligatorio**: è nel profilo minimo). ✅ **ORDINE DI RILASCIO — SODDISFATTO**: la 0010 è **applicata** al DB di produzione dal 26/07/2026 (registrata `20260726090451`; ricontrollato sul database vivo: `phone` e `city` nullable) `[V]`. Il vincolo «prima la migration, poi il form» è quindi assolto e il modulo si può alleggerire. *(Fino al 26/07 questa riga diceva «non applicata»: rimasta indietro di poche ore, corretta dopo verifica alla fonte.)* · profilo minimo = **nome, cognome, email+password (o accesso con Apple/Google), data di nascita per il 18+, consenso** — NB `email` non è colonna di `profiles`, vive in `auth.users` · 🔧 **il pezzo che conta**: il meccanismo che chiede il resto dopo (D-a) | Registrazione web completabile in **≤6 voci**; l'app continua a chiederle tutte; un profilo creato dal web ha `privacy_consent_at` valorizzato e una riga in `consent_events` |
