...
> 🔴 **PREMESSA CAMBIATA IL 2026-07-26 — leggere prima delle righe qui sotto.** Google e Apple sono
> stati **rimossi dall'accesso** (decisione di Riccardo, PR #120 in `master`): l'unico ingresso è
> email + password. Ogni riga che parla di alias `@privaterelay.appleid.com`, di «Nascondi la mia
> email» o di «privilegiare il social» descrive un mondo che non esiste più. Le righe interessate
> sono marcate **[SUPERATA 2026-07-26]** dove il verdetto cambia; le altre restano valide perché
> il ragionamento non dipendeva dal provider. Tre conseguenze che valgono per l'intera matrice:
> ① l'email che arriva a LD è **sempre reale e verificata** (nessun alias da riconciliare);
> ② **D2-bis smette di essere un buco Art.7**: nessun account può più nascere senza passare dal
> nostro form, che raccoglie consenso ed età minima prima di creare l'utente — è un miglioramento, non una
> riga da cancellare; ③ cade F-EMAIL.23 (la mail verificata che diventava quella dell'account) con
> tutto il suo costo: ri-aggancio post-conferma, dominio nelle «Email Sources» Apple, 4 segreti.
>
...
| claim `name` verso LD «quando disponibile» | Non «quando disponibile» e **nemmeno vuoto**: se la chiave manca, il server auth ci mette **l'EMAIL dell'account** come ripiego. Legge solo `user_metadata["name"]` (mai `full_name`), su entrambe le facce — id_token e UserInfo [V, codice Supabase Auth: `internal/tokens/service.go`, `internal/api/oauthserver/handlers.go`]. Per un utente Apple-hide il «nome» consegnato a LD sarebbe quindi il suo **alias `@privaterelay.appleid.com`**. Chiuso da **P1**. |
...
- **I2** L'identità si aggancia sul **`sub`**, mai sull'email. La motivazione originaria era l'alias
  Apple; caduto quello (2026-07-26), l'invariante **resta** per la ragione più generale: l'indirizzo
  cambia nel tempo, il `sub` no.
...
| **B2-bis** | *(il nodo)* Peso dell'onboarding | **Due vincoli distinti, da non confondere.** Lato **form**: 11 voci obbligatorie (asse 11) [V, `validation.ts:65-85`]. Lato **database**: `profiles` ha NOT NULL su `first_name`, `last_name`, `phone`, `city`, `birth_date`, `privacy_consent_at` + constraint di età [V, `0001_profiles.sql:4-17`; **soglia portata a 14 dalla migration 0019**, `province` reso nullable da `0007`]. Il form di LD chiede una manciata di voci (email, password, paese, consenso) [V] | Che chi vuole comprare una gift card non incontri il modulo più lungo della sua giornata | 🔧 **migration 0010**: rendere nullable `phone` e `city` (`last_name` **resta obbligatorio**: è nel profilo minimo). ✅ **ORDINE DI RILASCIO — SODDISFATTO**: la 0010 è **applicata** al DB di produzione dal 26/07/2026 (registrata `20260726090451`; ricontrollato sul database vivo: `phone` e `city` nullable) `[V]`. Il vincolo «prima la migration, poi il form» è quindi assolto e il modulo si può alleggerire. *(Fino al 26/07 questa riga diceva «non applicata»: rimasta indietro di poche ore, corretta dopo verifica alla fonte.)* · profilo minimo = **nome, cognome, email+password (o accesso con Apple/Google), data di nascita per l'età minima (14), consenso** — NB `email` non è colonna di `profiles`, vive in `auth.users` · 🔧 **il pezzo che conta**: il meccanismo che chiede il resto dopo (D-a) | Registrazione web completabile in **≤6 voci**; l'app continua a chiederle tutte; un profilo creato dal web ha `privacy_consent_at` valorizzato e una riga in `consent_events` |
| B3 | Ha **entrambi**, stessa email | Due account scollegati per sempre | Che LD colleghi i due sull'email | 📨 account linking (brief §4, 2° punto + domanda 3 di §5; nel documento di consegna è il punto 5) | Utente di test con account nativo → login RAH → un solo account |
| B4 | Ha **entrambi**, email diverse | Due account, non collegabili né sull'email né sul `sub` | Almeno una via manuale | **Lo scenario RESTA, cambia la causa**: non più l'alias Apple (impossibile dal 2026-07-26) ma il caso ordinario di chi da LD si era registrato con una casella che oggi non usa più. Meno frequente, non nullo · 📨 chiedere a LD se esiste un «collega il mio account» nel profilo utente · ⚠️ altrimenti residuo dichiarato | — |
...
| C2 | **[SUPERATA 2026-07-26]** Apple Private Relay | Lo scenario **non esiste più**: senza login Apple nessun account può nascere con un alias `@privaterelay.appleid.com`, e l'email dell'account è quella che la persona ha digitato e confermato | Che a LD arrivi l'indirizzo reale — **ottenuto, per costruzione** | ❌ **F-EMAIL.23 CADUTA** (era «l'indirizzo verificato diventa l'email dell'account»): con essa cadono il ri-aggancio post-conferma, la conferma al loro JIT sull'alias e il **prerequisito Apple «Email Sources»**, che era un blocco al rilascio. Il costo evitato è la parte che conta: 4 segreti da ruotare per sempre e un dominio da registrare presso Apple · ⚠️ resta il solo caso degli account Apple **nati prima** (al 2026-07-26: uno, di prova) | Registrazione di prova con email+password → il claim `email` porta l'indirizzo digitato |
...
| D2 | Nasce **da noi via web**, social (Apple/Google) | **Buco**: il trigger crea il profilo solo se `birth_date` è nei metadata — marker del form email; per il social il profilo lo crea `CompleteProfileScreen`, **che sul web non esiste** [V] → utente senza profilo, senza `privacy_consent_at`, senza `consent_events` = buco Art. 7 | Che ogni nascita, ovunque, lasci la stessa prova di consenso | 🔧 la pagina `/register` deve raccogliere l'età minima (14) e il consenso **prima** di creare l'utente e scrivere profilo + `consent_events` con la stessa semantica del trigger | Registrazione web via Apple → riga `consent_events` con `purpose='privacy_notice'` |
| **D2-bis** | ✅ **[CHIUSA 2026-07-26 dalla rimozione dei social]** — era il buco Art.7 già presente in app | **Storico**: chi entrava con Apple o Google creava `auth.users` subito, mentre profilo e `consent_events` nascevano solo completando «Completa profilo» → **chi abbandonava a metà restava senza `privacy_consent_at` e senza prova di consenso**, con un account già esistente. Combaciava con l'anomalia sul database: 4 consensi marketing e **zero** `privacy_notice`. **Oggi**: rimossi i provider, l'unica nascita passa dal form email+password, e il trigger `handle_new_user` scrive profilo **e** `consent_events` nella stessa transazione dell'insert in `auth.users` (`0011`, marker `birth_date`) — un account senza prova di consenso non può più esistere. **Non è un provvedimento preso: è un effetto collaterale buono di una decisione presa per altri motivi.** ⚠️ Restano gli account NATI PRIMA: la query di controllo va comunque eseguita una volta | Che nessun account esista senza la prova del consenso che lo legittima | 🔧 nulla da costruire per i nuovi · 🔧 resta il controllo una-tantum sui vecchi (2 utenti, entrambi di test al 2026-07-26) | Query: `auth.users` senza riga in `profiles` = 0, oppure ogni residuo ha meno di N giorni |
| D3 | Nasce **da loro** (form nativo) | Tre consensi loro: informativa, marketing titolare, marketing enti beneficiari [V] | — | ⚠️ residuo accettato: li qualifichiamo come titolari autonomi (in conferma legale, non acquisito), e di quell'utente non sappiamo nulla | — |
...
| D6 | **Età minima 14** *(era 18 fino al 30/07/2026)* | Constraint DB lato nostro [V, `eta_minima`, migration 0019] | Stesso presidio sul web | 🔧 `/register` verifica l'età prima dell'insert (riusa `validation.ts`, `MIN_AGE_YEARS`) | Data di nascita sotto i 14 → registrazione rifiutata |
...
| **D9** | ⚠️ **L'uscita che trasmette più dati è l'unica senza avviso** | La schermata onesta si mostra solo sul ramo Let's Donation, dove viaggia **soltanto** il codice di provenienza [V, `usePartnerExit.ts` → `openLetsDonationExit`/`exitLetsDonation`]. L'uscita verso Donorbox — che trasmette **nome, cognome ed email**, ma **solo a chi ha un profilo e ha accettato l'informativa corrente** (guardia aggiunta il 2026-07-25) — va dritta al browser senza mostrare nulla [V, `usePartnerExit.ts` → `openDonation`]. È l'opposto di come dovrebbe essere | Che l'avviso ci sia dove i dati trasmessi sono di più | 🔧 far passare anche `openDonation` dalla schermata (riuso del meccanismo esistente, nessun pezzo nuovo) · 🔑 è un cambio di comportamento visibile all'utente: decisione di Riccardo se farlo ora o al prossimo giro sulla UI. **Nel frattempo l'informativa dichiara il vero** (la schermata è annunciata solo per Let's Donation) | Uscita «Dona» dall'app → compare la schermata prima del browser |
...
| H4 | **Conferma email obbligatoria** — con «Confirm email» ON il signup **non restituisce sessione** [**A** — l'affermazione viene dal commento della migration `0004`, che la formula come condizione; il setting reale del progetto **non è stato letto**: verificarlo prima di derivarne scelte], quindi chi si registra dal web per rientrare su LD resta appeso alla mail | **[SUPERATA 2026-07-26 nel provvedimento, NON nel problema]** «Privilegiare Apple/Google» non è più un'opzione: i provider non ci sono più, e la conferma email è ora **l'unica** strada — il problema qui descritto passa da mitigabile a **strutturale**. Resta: schermata esplicita «controlla la posta» + ripresa del rientro dopo la conferma. E diventa bloccante il prerequisito di posta (SMTP proprio + DKIM/SPF/DMARC): con la password unica via d'ingresso, una mail che non arriva è una persona che non entra. Premessa `[A]` ora **verificata**: la conferma email era ed è ON (`confirmation_sent_at` + `email_confirmed_at` a un'ora di distanza sull'utente del 21-06) | Registrazione email dal web → il rientro su LD non si perde |
...
| **H11** | **LD riceve un access token con pieni poteri sull'utente** — nel flusso OIDC standard il client ottiene al token-endpoint un access token Supabase che **non è limitato dagli scope**: l'autorizzazione dipende interamente dalle RLS, quindi con quel token LD potrebbe leggere via API i dati di quell'utente. **Erode di fatto l'invariante I1** («nessun accesso reciproco ai database»): I1 va letto come «nessun accesso diretto e permanente», non come «nessun dato raggiungibile». Era noto (piano OIDC) ma mancava qui | 🔧 audit RLS riga per riga prima di accendere · 📨 LD si impegna a leggere l'identità **solo** da id_token/UserInfo, mai a chiamare le nostre API · 🔧 TTL degli access token il più corto possibile (NB: su Supabase è impostazione di progetto, non per-client) · 🔑 metterlo nero su bianco nell'accordo di condivisione dati | Prova pratica: con un access token di test, una chiamata alle nostre API deve restituire solo le righe di quell'utente |
...
| **R7** | Domande di chiarimento: ~~alias Apple accettato dal JIT?~~ (caduta col login Apple, 2026-07-26) `email_verified` richiesto? Anagrafica aggiornata a ogni login o solo al JIT? Marketing a false sul JIT? Informativa LD mostrata all'account JIT? Cancellazione propagabile? Che dati chiede l'iscrizione a un evento? | C2, C3, C4, D4, D5, D7, E4 |
| **R8** 🔝 | **Il precedente Zucchetti — PROMOSSA a prima domanda** (correzione Riccardo 2026-07-25): loro dicono che dal portale Zucchetti i dipendenti «arrivano già registrati». Se quel meccanismo è un SSO, **hanno già la cosa che stiamo chiedendo** e R3/R5 non sono più «costruire», sono «riusare con noi al posto di Zucchetti» — il che smonta l'obiezione «SSO-only lo vedo difficile». Era in coda al brief (§6, dopo le domande) perché l'avevo trattata come curiosità: errore di collegamento, non di contenuto. Ora §2.3 + domanda 1. Ipotesi da confermare, NON dedotta come fatto: sul tenant pubblico Zucchetti il login è email+password e in registrazione c'è «Gruppo Aziendale» → potrebbe essere pre-associazione dell'azienda, non SSO [A] | A1, B1, R3, R5 |
...
| **P3** | **Pagina web** (Next.js): `/consent`, `/register` (con età minima e consenso **prima** dell'insert, profilo e `consent_events` nello stesso atto), `/auth/callback`, rilevamento in-app browser | Hosting (leva) |
...
2. Chi da LD ha un account con un **indirizzo diverso** da quello che usa da noi resta con **due
   account** (B4). Fino al 2026-07-26 la causa principale erano gli utenti Apple-hide; rimossi i
   social, resta il caso ordinario di chi ha cambiato casella. Residuo piu' raro, non chiuso.
...
8. **Let's Donation riceve comunque un access token con pieni poteri** sull'utente che accede: è
   inevitabile nel flusso OIDC standard, non è una scelta nostra. Si mitiga (RLS, TTL, impegno
   contrattuale) ma non si elimina — H11.

### 3.4 Le decisioni di Riccardo — PRESE il 2026-07-25

| # | Decisione | Esito |
| --- | --- | --- |
| **D-a** | Quali campi nella registrazione web | **«Servono tutti quei dati. Prima o poi.»** → profilo **minimo alla nascita** (nome, email, data di nascita per l'età minima, consenso) **+ completamento OBBLIGATORIO differito**. Attenzione al fraintendimento: *nullable ≠ opzionale per sempre* — senza un meccanismo che chiuda il profilo alla prima occasione utile (rientro in app, iscrizione a un evento), quei dati non arriverebbero mai. La migration 0010 è la condizione tecnica, **non** il provvedimento completo |
