...
| **B2-bis** | *(il nodo)* Peso dell'onboarding | **Due vincoli distinti, da non confondere.** Lato **form**: 11 voci obbligatorie (asse 11) [V, `validation.ts:65-85`]. Lato **database**: `profiles` ha NOT NULL su `first_name`, `last_name`, `phone`, `city`, `birth_date`, `privacy_consent_at` + constraint 18+ [V, `0001_profiles.sql:4-17`, `province` reso nullable da `0007`]. Il form di LD chiede una manciata di voci (email, password, paese, consenso) [V] | Che chi vuole comprare una gift card non incontri il modulo più lungo della sua giornata | 🔧 **migration 0010**: rendere nullable `phone` e `city` (`last_name` **resta obbligatorio**: è nel profilo minimo). ⚠️ **ORDINE DI RILASCIO**: la 0010 è in `master` ma **non applicata al DB di produzione** (l'`alter … drop not null` è bloccato dal guard MCP → SQL Editor del Dashboard, leva umana). Alleggerire il form **prima** dell'apply manda in produzione un signup che **viola il NOT NULL e fallisce**: prima la migration, poi il form · profilo minimo = **nome, cognome, email+password (o accesso con Apple/Google), data di nascita per il 18+, consenso** — NB `email` non è colonna di `profiles`, vive in `auth.users` · 🔧 **il pezzo che conta**: il meccanismo che chiede il resto dopo (D-a) | Registrazione web completabile in **≤6 voci**; l'app continua a chiederle tutte; un profilo creato dal web ha `privacy_consent_at` valorizzato e una riga in `consent_events` |
