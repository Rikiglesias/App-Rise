...
| C5 | **Claim `name` vuoto** | Per ogni utente email/password `raw_user_meta_data` **non contiene `name`** [V, `useSignUpForm.ts:154-159`]. Che da lì discenda un claim `name` vuoto è **[A]**: come il server OIDC costruisca quel claim (`name`? `full_name`?) va letto sulla sua documentazione **prima** di eseguire P1, o si scriverebbe la chiave sbagliata | Nome presente per tutti | 🔧 popolare `user_metadata.name` — backfill degli esistenti + scrittura al signup e a ogni aggiornamento profilo | Query: `auth.users` con `raw_user_meta_data->>'name'` non nullo = 100% |
