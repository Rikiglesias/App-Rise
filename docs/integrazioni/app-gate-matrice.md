...
| **C3** | **Completamento del profilo parziale** (costruito ieri, PR #93) | `getProfileCompletion` è consumato **solo** in `ProfileScreen`; `missingProfileFields` — la lista ordinata di cosa manca — non è chiamata direttamente da nessuna schermata, solo attraverso di esso [V]. **AGGIORNAMENTO 2026-07-26 (PR #112)**: la lista non è più `phone`/`city`/`province` ma include **`contact_email`** → il perimetro di C3 si allarga di molto, perché un profilo nato da registrazione email/password ha quella colonna vuota (finché la migration 0011 non è applicata) e risulta quindi **incompleto**: il sollecito non riguarda più solo chi ha saltato il telefono. Con esso vale il residuo **F-EMAIL.29**: la scheda profilo non ha una riga per la mail di contatto, quindi chi ha solo quella mancante vede «Completa il tuo profilo» senza capire cosa gli si chiede | solo chi apre il profilo | 🔧 **il pezzo che manca a P2**: il sollecito va dove la persona passa (home, o prima dell'azione che quei dati richiede), non dove potrebbe non entrare mai · 🔑 dove esattamente = decisione di design | Utente con profilo parziale che **non** apre il profilo: incontra la richiesta? Oggi **no** |
