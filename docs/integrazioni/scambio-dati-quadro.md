...
| ⚠️ **Vincolo di ordine, non tecnico ma operativo** | — | — | **Corretto il 2026-07-27**: qui si leggeva che il collegamento scatta «solo quando il profilo nasce», e non è così. Scatta anche **ogni volta che la persona salva il proprio profilo** — quindi chi si è registrato prima del caricamento si ricollega da solo al primo salvataggio, senza che nessuno debba fare niente (verificato sulla documentazione del database e provato con un test apposta). **Il vincolo resta, ma riguarda meno persone**: chi non riapre mai il profilo non si ricollega mai. Quindi **il riordino va comunque fatto prima che comincino le registrazioni vere** — prima del primo rilascio, dopo l'informativa — ma se qualcuno si registrasse nel mezzo il danno non è definitivo. La passata di ricollegamento una-tantum servirebbe solo per chi non torna, e oggi non esiste. Da non confondere con il **pre-caricamento di anagrafiche verso il partner** (§5.1): quello è il verso opposto, noi → loro, ed è escluso | — | `[V]` |
...
| «Vi arriva sempre il nome» | Poteva mancare per chi entrava con Apple nascondendo la mail. **Caso chiuso per i nuovi** (accesso solo con email e password dal 26/07/2026): il nome è un campo obbligatorio del nostro modulo. Resta possibile sugli account nati prima | «Il nome arriva se disponibile: il vostro sistema deve tollerarne l'assenza» |
...
| **Caricare le anagrafiche storiche PRIMA che comincino le registrazioni vere** (§3.1, vincolo di ordine). Chi si registra prima del caricamento si ricollega da solo al primo salvataggio del proprio profilo (corretto il 27/07/2026: prima qui si diceva «solo quando il profilo nasce», ed era sbagliato) — ma **chi non riapre mai il profilo resta scollegato, e nessuno se ne accorge**. Il meccanismo esiste, è provato (**21 prove** per la `0012` + **12** per la `0013`, cioè 22 e 15 righe verdi contando la riga d'esito, su due configurazioni di privilegi opposte) ed è ora **acceso sul database vivo: `0012` e `0013` applicate il 27/07/2026, nell'ordine, e ricontrollate alla fonte** `[V]` (l'elenco delle modifiche le riporta entrambe; il testo delle tre funzioni installate coincide alla lettera con quello del progetto). **Resta da fare il caricamento**, che è la cosa vera: finché la tabella è vuota il comportamento è identico a prima. Serve prima la riga qui sopra | Noi (caricamento) |
