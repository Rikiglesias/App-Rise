...
| **Google e Apple** | Titolari autonomi | L'identificativo e l'email con cui la persona accede; con Apple, eventualmente un alias di inoltro | Nulla del nostro database: sanno che quella persona ha un account nostro. **È l'unico caso in cui un dato personale ci entra da un terzo**, ed è da qui che nasce il problema dell'alias che condiziona metà delle scelte di questo documento `[V]` |
