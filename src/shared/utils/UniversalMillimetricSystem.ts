...
export const calculateMillimetricSize = (referenceValue: number): number => {
  ...
  const finalScale = Math.max(minScale, Math.min(maxScale, scaledValue / referenceValue));
  return Math.round(referenceValue * finalScale * 100) / 100; // Precisione decimale
};

// 🎯 FUNZIONI SPECIFICHE PER OGNI TIPO
export const universalFont = (fontSize: number): number => calculateMillimetricSize(fontSize);
export const universalSpacing = (spacing: number): number => calculateMillimetricSize(spacing);
export const universalWidth = (width: number): number => calculateMillimetricSize(width);
export const universalHeight = (height: number): number => calculateMillimetricSize(height);
