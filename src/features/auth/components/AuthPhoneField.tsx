...
/** Handle esposto via ref: consente al form di spostare il focus su questo campo
 * (es. keyboard "next" dal campo precedente). Espone solo focus(), non tutta la
 * superficie TextInput. */
export interface AuthPhoneFieldHandle {
  focus: () => void;
}
 ...
 */
const AuthPhoneFieldImpl = forwardRef<
  AuthPhoneFieldHandle,
  AuthPhoneFieldProps
>(
  (
    {
    ...
    // Ref al PhoneInput della libreria (forwardRef, espone focus/blur/clear): il form
    // ci sposta il focus via keyboard "next" dal campo precedente (finding 288).
...
AuthPhoneFieldImpl.displayName = 'AuthPhoneField';
