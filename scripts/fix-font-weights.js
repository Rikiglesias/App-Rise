...
const FIXES = [
  ...
  // 2. Aggiunge fontWeight props ai PerfectText senza fontWeight
  {
    ...
    pattern: /<PerfectText([^>]*?)(?<!fontWeight=[^>]*?)(\s+size=\{?\d+\}?\s+lines=\{?\d+\}?)([^>]*?)>/g,
...
function analyzeFile(filePath) {
  ...
  const withoutFontWeight = perfectTextMatches.filter(match => 
    !match.includes('fontWeight=')
  );
...
function main() {
  ...
  if (!CONFIG.dryRun && results.modifiedFiles > 0) {
    ...
    console.log('💡 I font dovrebbero ora essere più uniformi tra le piattaforme.');
  }
