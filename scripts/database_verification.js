...
const testDevices = [
  ...
  { model: 'iPhone 15', width: 393, expected: 42.0 },
  {
    ...
    width: 430,
    expected: calculateMillimetricFontSize(430),
  },
  {
    model: 'iPhone 14',
    width: 390,
    expected: calculateMillimetricFontSize(390),
  },
  {
    model: 'iPhone 11',
    width: 414,
    expected: calculateMillimetricFontSize(414),
  },
  ...
  // Samsung - Corretti
  {
    model: 'Galaxy S24',
    width: 360,
    expected: calculateMillimetricFontSize(360),
  },
  {
    model: 'Galaxy S23 Ultra',
    width: 384,
    expected: calculateMillimetricFontSize(384),
  },
  ...
  // Xiaomi - Corretti
  {
    model: 'Redmi Note 13',
    width: 393,
    expected: calculateMillimetricFontSize(393),
  },
  ...
  // Test dispositivi con scaling limits
  {
    model: 'OnePlus 12',
    width: 450,
    expected: calculateMillimetricFontSize(450),
  },
];
...
testDevices.forEach(device => {
  ...
  if (device.error) {
    console.log(
      `❌ ${device.model}: ${device.width}px → ${calculated.toFixed(3)}px (${device.error})`
    );
    errorsFound++;
  } else if (isCorrect) {
    console.log(
      `✅ ${device.model}: ${device.width}px → ${calculated.toFixed(3)}px`
    );
  } else {
    console.log(
      `❌ ${device.model}: ${device.width}px → ${calculated.toFixed(3)}px (Expected: ${device.expected.toFixed(3)})`
    );
    errorsFound++;
  }
});
...
scalingTests.forEach(test => {
  ...
  console.log(
    `${isCorrect ? '✅' : '❌'} ${test.width}px → ${calculated.toFixed(3)}px (${test.description})`
  );
  if (!isCorrect) {
    errorsFound++;
  }
...
console.log(
  '1. Google Pixel: CORRETTI - Ora usano dimensioni CSS logiche (412px)'
);
console.log('2. Pixel 9: 412px → 44.24px font (CORRETTO)');
console.log('3. Pixel Fold: 673px → 58.8px font (CORRETTO - limite massimo)');
console.log(
  '4. Tutti i Pixel ora usano 412px (dimensioni CSS logiche standard)'
);
...
console.log(
  `Successo: ${(((totalTests - errorsFound) / totalTests) * 100).toFixed(1)}%`
);
